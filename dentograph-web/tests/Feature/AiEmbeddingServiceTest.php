...
test('ai embedding service embeds multiple texts in one request', function () {
    config()->set('services.ai_embedding.url', 'http://127.0.0.1:8001');
    ...
    $result = app(AiEmbeddingService::class)->embedMany(['chunk satu', 'chunk dua']);

    expect($result['model'])->toBe('bge-m3:567m')
        ->and($result['dimensions'])->toBe(1024)
        ->and($result['embeddings'])->toHaveCount(2)
        ->and($result['embeddings'][0])->toHaveCount(1024)
        ->and($result['embeddings'][1])->toHaveCount(1024);

    Http::assertSent(fn ($request): bool => $request['texts'] === ['chunk satu', 'chunk dua']);
});
